...
const validationJwtRol = (rol)=>{
    return async (req=request,res=response,next)=>{
        ...
        if(!token){
            return responseFunc.error(req,res,"Algo salió mal",401,"No hay token")
        }
        try{
            ...
            if(rol && user.rol !== rol){
                return responseFunc.error(req,res,"Algo salió mal",401,"No tienes permiso para esta operación")
            }
            req.user = user;
            next()
        } catch(error){
